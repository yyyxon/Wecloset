...
var calendar = $('#calendar').fullCalendar({
 ...
 /** ******************
  ...
  navLinks                  : true,
  defaultDate               : moment(getThisMonth()), //실제 사용시 현재 날짜로 수정
  timeFormat                : 'HH:mm',
  ...
  events: function (start, end, timezone, callback) {
	...
	//1604188800000 :  1607817600000 :  local :  function(t){i.calendar.popLoading(),r(i.parseEventDefs(t))}
    $.ajax({
      type: "post",
      ...
      success: function (response) {
		...
		var fixedDate =$.each(response.calendarList, function(index, array){
		...
		callback(fixedDate);
	     ...
	     //핸들바 템플릿 가져오기
       // var source = $("#calendarUserTemplate").html();	 		
       // var template = Handlebars.compile(source);			  
       // $("#calendarUserList").append(template(response.userList));             	 	
  ...
  eventDrop: function (event, delta, revertFunc, jsEvent, ui, view) {
	...
	if(event.allDay){
		var resultEndDay=  moment( newDates.endDate).add(1, 'days').format('YYYY-MM-DD HH:mm');
		eventDropEventData = {
	        _id: event._id,
	        start: newDates.startDate,
	        end: resultEndDay,
	...
	//console.log('수정: ' + newDates.startDate + ' ~ ' + newDates.endDate + " event.allDay : " +event.allDay);
...
*/
    ...
    $.ajax({
        ...
        data: {
            ...
            eventData:JSON.stringify(eventDropEventData)
        },
		dataType : 'json',
        success: function (response) {
...
function editColorChange(){	
	var str=$("#edit-type").val();
